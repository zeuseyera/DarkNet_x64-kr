							...
							, ACTIVATION activation )
{
	...
	l.n = n;	// 포집(커널) 개수
	l.batch		= batch;	// 사리수
	l.stride	= stride;	// 포집 보
	l.size		= size;		// 포집(커널) 가로세로 크기
	l.pad		= pad;		// 패딩

	int out_h = local_out_height( l );	// 출럭 세로크기
	int out_w = local_out_width( l );	// 출력 가로크기
