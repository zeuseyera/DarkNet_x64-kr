...
void mean_delta_cpu( float *delta, float *variance, int batch, int filters, int spatial, float *mean_delta )
{
	...
	for ( i = 0; i < filters; ++i )
	{
		...
		mean_delta[i] *= (-1.0/sqrt( variance[i] + 0.00001f ));
	}
}
						...
						, float *variance_delta )
{
	...
	for ( i = 0; i < filters; ++i )
	{
		...
		variance_delta[i] *= -0.5 * pow( variance[i] + 0.00001f, (float)(-3.0/2.0) );
	}
}
	...
	for ( j = 0; j < batch; ++j )
	{
		for ( f = 0; f < filters; ++f )
		{
			for ( k = 0; k < spatial; ++k )
			{
				...
				delta[index] = ( delta[index] * 1.0 ) / ( sqrt( variance[f] + 0.00001f ) )
							 + ( variance_delta[f] * 2.0 * (x[index] - mean[f]) ) / (spatial * batch)
...
void forward_batchnorm_layer( layer l, network net )
{
	...
	if ( net.train )
	{
		...
		scal_cpu( l.out_c, 0.99f, l.rolling_mean, 1 );
		axpy_cpu( l.out_c, 0.01f, l.mean, 1, l.rolling_mean, 1 );
		scal_cpu( l.out_c, 0.99f, l.rolling_variance, 1 );
		axpy_cpu( l.out_c, 0.01f, l.variance, 1, l.rolling_variance, 1 );
...
void backward_batchnorm_layer_gpu( layer l, network net )
{
	...
	fast_variance_delta_gpu( l.x_gpu
					, l.delta_gpu
					, l.mean_gpu
					, l.variance_gpu
					, l.batch
