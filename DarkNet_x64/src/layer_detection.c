	...
	if ( l.softmax )
	{
		for ( b = 0; b < l.batch; ++b )
		{
			...
			for ( i = 0; i < locations; ++i )
			{
				...
				softmax( l.output + index + offset	// 입력값
					, l.classes						// 개수
					, 1								//
					, 1								// 보
					, l.output + index + offset );	// 출력값
			}
		}
	}
			...
			for ( i = 0; i < locations; ++i )
			{
				...
				for ( j = 0; j < l.classes; ++j )
				{
					...
					*(l.cost) += l.class_scale
								* pow( net.truth[truth_index+1+j] - l.output[class_index+j], 2 );
				...
				*(l.cost) += l.object_scale * pow( 1-l.output[p_index], 2 );
				...
				if ( l.sqrt )
				{
					l.delta[box_index+2] = l.coord_scale*(sqrt( net.truth[tbox_index + 2] )
										- l.output[box_index + 2]);
					l.delta[box_index+3] = l.coord_scale*(sqrt( net.truth[tbox_index + 3] )
