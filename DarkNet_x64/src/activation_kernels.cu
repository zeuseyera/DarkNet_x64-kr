...
// 신경망 출력값을 계산한다
extern "C" void activate_array_ongpu( float *x			// 입력값(신경망 출력값)
									, int GaeSu			// 계산할 원소개수
									, ACTIVATION a )	// 
{
    activate_array_kernel<<<cuda_gridsize(GaeSu), BLOCK>>>( x, GaeSu, a );
    check_error(cudaPeekAtLastError());
}
// 신경망 출력값에 대한 기울기를 계산한다
extern "C" void gradient_array_ongpu( float *x			// 입력값(신경망 출력값)
									, int GaeSu			// 계산할 원소개수
									, ACTIVATION a		// 
									, float *delta )	// 계산된 기울기값
{
    gradient_array_kernel<<<cuda_gridsize(GaeSu), BLOCK>>>( x, GaeSu, a, delta );
    check_error(cudaPeekAtLastError());
}
