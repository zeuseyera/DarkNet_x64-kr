...
// GRU(Gated Recurrent Unit, 문달린 재사용 장치)
...
layer make_gru_layer( int batch, int inputs, int outputs, int steps, int batch_normalize )
{
	...
	*(l.input_z_layer) = make_connected_layer( batch*steps
											, inputs
											, outputs
											, LINEAR
											, batch_normalize );
	...
	*(l.state_z_layer) = make_connected_layer( batch*steps
											, outputs
											, outputs
											, LINEAR
											, batch_normalize );
	...
	fprintf( stderr, "\t\t" );
	*(l.input_r_layer) = make_connected_layer( batch*steps
											, inputs
											, outputs
											, LINEAR
											, batch_normalize );
	...
	*(l.state_r_layer) = make_connected_layer( batch*steps
											, outputs
											, outputs
											, LINEAR
											, batch_normalize );
	...
	*(l.input_h_layer) = make_connected_layer( batch*steps
											, inputs
											, outputs
											, LINEAR
											, batch_normalize );
	...
	*(l.state_h_layer) = make_connected_layer( batch*steps
											, outputs
											, outputs
											, LINEAR
											, batch_normalize );
...
void backward_gru_layer_gpu( layer l, network_state state )
{
	...
	for ( i = l.steps-1; i >= 0; --i )
	{
		...
		weighted_delta_gpu( l.prev_state_gpu
						, l.h_gpu
						, l.z_gpu
						, prev_delta_gpu
		...
		if ( prev_delta_gpu )
			mult_add_into_gpu( l.outputs*l.batch
							, l.forgot_delta_gpu
							, l.r_gpu
							, prev_delta_gpu );
