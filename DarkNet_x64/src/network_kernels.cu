...
void forward_network_gpu( network net, network_state state )
{
	...
	for ( i=0; i<net.n; ++i )
	{
		...
		// 각 층의 출력개수*사리수 개수만큼 layer.delta_gpu에 0으로 채운다
		if ( l.delta_gpu )
		{
		...
		// 현재층의 출력값 주소를 다음층의 입력값 주소에 지정한다
...
void forward_backward_network_gpu( network net, float *x, float *y )
{
	...
	state.index = 0;
	state.net = net;
	int x_size = get_network_input_size( net )*net.batch;		// 입력값 개수 * 사리수
	int y_size = get_network_output_size( net )*net.batch;	// 출력값 개수 * 사리수
	...
	forward_network_gpu( net, state );	// 신경망 순방향 계산
	backward_network_gpu( net, state );	// 신경망 역방향 계산
}
//
float train_network_datum_gpu( network net, float *x, float *y )
{
	*net.seen += net.batch;
	...
	if ( ((*net.seen) / net.batch) % net.subdivisions == 0 )
		update_network_gpu( net );	// 신경망 가중값 갱신
...
float *network_predict_gpu( network net, float *input )
{
	...
	state.index = 0;
	state.net = net;
	// 장치에서 계산에 필요한 장치메모리를 할당하고 사비값을 복사한다
	state.input = cuda_make_array( input, size );
	state.truth = 0;
	state.train = 0;
	state.delta = 0;

	forward_network_gpu( net, state );	// 신경망 순방향 출력계산
