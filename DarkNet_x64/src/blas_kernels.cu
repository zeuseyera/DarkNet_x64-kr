...
{
	dot_kernel<<<cuda_gridsize(l.n*l.n), BLOCK>>>( l.output_gpu
												, l.dot
												, l.batch
												, l.n
												, l.out_w * l.out_h
...
extern "C" void adam_update_gpu( float *w
							, float *d
							, float *m
							, float *v
							, float B1
...
extern "C" void normalize_delta_gpu( float *x
								, float *mean
								, float *variance
								, float *mean_delta
								, float *variance_delta
								...
								, float *delta )
{
	...
	normalize_delta_kernel<<<cuda_gridsize( N ), BLOCK>>>( N
														, x
														, mean
														, variance
														, mean_delta
...
extern "C" void mean_delta_gpu( float *delta
							, float *variance
							, int batch
							, int filters
							, int spatial
							, float *mean_delta )
{
	mean_delta_kernel<<<cuda_gridsize( filters ), BLOCK>>>( delta
														, variance
														, batch
														, filters
														, spatial
...
extern "C" void fast_mean_delta_gpu( float *delta
	, float *variance
	, int batch
	, int filters
	, int spatial
...
extern "C" void fast_variance_delta_gpu( float *x
									, float *delta
									, float *mean
									, float *variance
									, int batch
									...
									, float *variance_delta )
{
	fast_variance_delta_kernel<<<filters, BLOCK>>>( x
												, delta
												, mean
												, variance
												, batch
...
extern "C" void fast_variance_gpu( float *x
								, float *mean
								, int batch
								, int filters
								, int spatial
...
extern "C" void axpy_gpu_offset( int N
							, float ALPHA
							, float * X
							, int OFFX
							, int INCX
							...
							, float *out )
{
	...
	shortcut_kernel<<<cuda_gridsize( size ), BLOCK>>>( size
													, minw
													, minh
													, minc
													, stride
...
// SSE 오류를 계산한다
__global__ void l2_kernel( int nn, float *pred, float *truth, float *delta, float *error )
{
	int ii = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
	if ( ii < nn )
	{
		float diff = truth[ii] - pred[ii];
		error[ii] = diff * diff; //I know this is technically wrong, deal with it.
		delta[ii] = diff;
	}
}
// SSE 오류를 계산한다
extern "C" void l2_gpu( int nn, float *pred, float *truth, float *delta, float *error )
{
	l2_kernel<<<cuda_gridsize( nn ), BLOCK>>>(nn, pred, truth, delta, error);
	check_error( cudaPeekAtLastError() );
}
...
extern "C" void softmax_tree( float *input
							, int spatial
							, int batch
							, int stride
							, float temp
							...
							, tree hier )
{
	...
	*/
	...
	softmax_tree_kernel<<<cuda_gridsize( num ), BLOCK>>>( input
		, spatial
		, batch
		, stride
		, temp
							...
							, float *output )
{
	...
	int b = id / groups;
	int g = id % groups;
	softmax_device( input + b*batch_offset + g*group_offset
				, n
				, temp
				, stride
...
extern "C" void softmax_gpu( float *input
						, int n
						, int batch
						, int batch_offset
						, int groups
						...
						, float *output )
{
	softmax_kernel<<<cuda_gridsize( batch*groups ), BLOCK>>>( input
															, n
															, batch
															, batch_offset
															, groups
...
extern "C" void upsample_gpu( float *in
							, int w
							, int h
							, int c
							, int batch
