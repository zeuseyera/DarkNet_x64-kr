...
{
	int i, j, b;
	int k = get_network_output_size( net );						// 출력개수
	matrix pred = make_matrix( test.X.rows, k );				// 반환값 배열 생성
	float *X = calloc( net.batch*test.X.cols, sizeof( float ) );	// 입력값 배열 생성
