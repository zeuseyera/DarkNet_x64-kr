...
void draw_detections_cv( IplImage* show_img
					, int num
					, float thresh
					, box *boxes
					, float **probs
		...
		, char			*out_filename )
{
	...
	{
		printf( "video file: %s\n", filename );
		cap = cvCaptureFromFile( filename );	// ffmpeg.dll 파일이 있어야 함
	}
	else
	{
	...
	while ( 1 )
	{
		...
		if ( 1 )
		{
			...
			else
			{
				...
				sprintf_s( buff, 256, "%s_%08d", prefix, count );
