...
#include "../Net/layer_region.h"
#include "../Net/layer_cost.h"
...
void test_detector( char *datacfg	// 자료파일 설정정보 파일
				, char *cfgfile
				, char *weightfile
				, char *filename	// 검출에 사용할 그림파일
				, float thresh )
{
	list *options		= read_data_cfg( datacfg );
	char *name_list		= option_find_str( options, "names", "data/names.list" );
	char **names		= get_labels( name_list );
	// 검출이미지에 문자를 그리기 위해, 알파벳 아스키 이미지를 탑재한다
	...
	if ( weightfile )
	{
		load_weights( &net, weightfile );	// 파일로 저장된 가중값을 탑재한다
	}
	...
	while ( 1 )
	{
		...
		image im	= load_image_color( input, 0, 0 );	// 사진을 메모리에 탑재한다
		image sized	= resize_image( im, net.w, net.h );	// 사진을 신경망 입력크기로 조정한다
		layer l		= net.layers[net.n-1];	//출력층 지정(
		//경게구역 개수: 5 ( layer.n )
		box *boxes = calloc( l.w*l.h*l.n, sizeof( box ) );
		...
		float *X = sized.data;
		time=clock();
		network_predict( net, X );	// 신경망 출력을 계산한다

		//printf( "%s: Predicted in %f seconds.\n", input, sec( clock()-time ) );
		printf( "%s: 검출에 %f 초.\n", input, sec( clock()-time ) );
		// 신경망 출력으로 예측한 경계구역을 가져온다
		get_region_boxes( l, 1, 1, thresh, probs, boxes, 0, 0 );
		//
		if ( nms )
			do_nms_sort( boxes, probs, l.w*l.h*l.n, l.classes, nms );
		// 검출이미지에 문자를 그린다, 분류개수: 20 (layer.classes)
