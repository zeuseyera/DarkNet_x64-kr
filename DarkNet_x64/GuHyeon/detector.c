...
*/
		...
		if ( i%100==0 )
		{
			...
			char buff[256];
			//sprintf( buff			//  [7/3/2018 jobs]
			sprintf_s( buff, 256	//  [7/3/2018 jobs]
				, "%s/%s.backup"
...
void validate_detector_flip( char *datacfg, char *cfgfile, char *weightfile, char *outfile )
{
	...
	if ( 0==strcmp( type, "coco" ) )
	{
		...
		//fp = fopen( buff, "w" );		//  [7/3/2018 jobs]
		fopen_s( &fp, buff, "w" );		//  [7/3/2018 jobs]
		fprintf( fp, "[\n" );
		coco = 1;
	}
	...
	else
	{
		...
		for ( j = 0; j < classes; ++j )
		{
			...
			//fps[j] = fopen( buff, "w" );	//  [7/3/2018 jobs]
			fopen_s( &fps[j], buff, "w" );	//  [7/3/2018 jobs]
		}
	}
...
void validate_detector_recall( char *cfgfile, char *weightfile )
{
	...
	for ( i = 0; i < m; ++i )
	{
		...
		int nboxes	= 0;
		detection *dets = get_network_boxes( net, sized.w, sized.h, thresh, 0.5f, 0, 1, &nboxes );
		...
		fprintf( stderr
			...
			, i, correct, total, (float)proposals/(i+1), avg_iou*100/total, 100.0f*correct/total );
...
/*
void censor_detector(char *datacfg
				, char *cfgfile
				, char *weightfile
				, int cam_index
				, const char *filename
...
void extract_detector( char *datacfg
					, char *cfgfile
					, char *weightfile
					, int cam_index
					, const char *filename
...
void run_detector( int argc, char **argv )
{
    ...
    float thresh		= find_float_arg( argc, argv, "-thresh", 0.5f );	// 문턱값
    float hier_thresh	= find_float_arg( argc, argv, "-hier", 0.5f );		//
    int cam_index		= find_int_arg( argc, argv, "-c", 0 );				// 카메라 지적
	...
	if ( argc < 4 )
	{
		...
		//	, "usage: %s %s [train/test/valid] [cfg] [weights (optional)]\n"	//  [7/12/2018 jobs]
			, "사용방법: %s %s [train/test/valid] [구성파일] [가중값파일 (선택항목)]\n"	//  [7/12/2018 jobs]
			, argv[0], argv[1] );
		return;
	}
	...
	char *cfg		= argv[4];					// 망구성 파일
	char *weights	= (argc > 5) ? argv[5] : 0;	// 가중값 파일
	char *filename	= (argc > 6) ? argv[6] : 0;	// 입력 파일

    if		( 0==strcmp(argv[2], "test") )		test_detector( datacfg
															, cfg
															, weights
															, filename
															, thresh
	...
	/*
	else if ( 0==strcmp( argv[2], "extract" ) )	extract_detector( datacfg
																, cfg
																, weights
																, cam_index
																, filename
																...
																, thresh
																, frame_skip );
	else if ( 0==strcmp( argv[2], "censor" ) )	censor_detector( datacfg
																, cfg
																, weights
																, cam_index
																, filename
