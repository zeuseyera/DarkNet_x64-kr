...
void train_classifier( char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear )
{
	...
	args.w			= net->w;	// 망 사비 너비
	args.h			= net->h;	// 망 사비 높이
	//args.threads	= 32;
	args.threads	= 1;
	...
	args.paths		= paths;	// 수련자료 파일이름
	args.classes	= classes;	// 분류개수
	args.n			= imgs;		// 사비 사리수
	args.m			= N;		// 벼림 사비자료 총 개수
	args.labels		= labels;	// 분류 이름목록
	...
	int count	= 0;
	*net->seen	= 0;	// 벼림자료추적 초기화 [7/28/2018 jobs]
	int epoch	= (int)(*net->seen/N);

	printf( "현사리,   추적:    오차,     오차평균,       벼림비,      계산시간,     추적 이미지\n" );
