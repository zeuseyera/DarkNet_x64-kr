...
/*
#ifdef WIN32
#include <time.h>		// 윈도우 환경 [7/4/2018 jobs]
#else
#include <sys/time.h>	// 유닉스 환경 [7/4/2018 jobs]
#endif
*/
...
void train_attention( char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear )
{
	...
	//srand( time( 0 ) );
	srand( (unsigned int)time( 0 ) );	//  [6/28/2018 jobs]
	int seed = rand();
	...
	int epoch = (int)( (*net->seen)/N );	//  [6/28/2018 jobs]
...
*/
		...
		avg_cls_loss = avg_cls_loss*0.9f + closs*0.1f;
		avg_att_loss = avg_att_loss*0.9f + aloss*0.1f;
		...
		{
			//epoch = *net->seen/N;
			epoch = (int)( (*net->seen)/N );	//  [6/28/2018 jobs]
			char buff[256];
			//sprintf( buff, "%s/%s_%d.weights", backup_directory, base, epoch );	//  [6/26/2018 jobs]
		...
		{
			char buff[256];
			//sprintf( buff, "%s/%s.backup", backup_directory, base );	//  [6/26/2018 jobs]
	...
	char buff[256];
	//sprintf( buff, "%s/%s.weights", backup_directory, base );	//  [6/26/2018 jobs]
...
void validate_attention_single( char *datacfg, char *filename, char *weightfile )
{
	...
	//srand( time( 0 ) );
	srand( (unsigned int)time( 0 ) );	//  [6/27/2018 jobs]
	...
	for ( i = 0; i < m; ++i )
	{
		...
		image crop = crop_image( resized
							, (resized.w - net->w*divs/size)/2
							, (resized.h - net->h*divs/size)/2
...
void predict_attention( char *datacfg, char *cfgfile, char *weightfile, char *filename, int top )
{
	...
	while ( 1 )
	{
		if ( filename )
		{
			//strncpy( input, filename, 256 );	//  [6/26/2018 jobs]
