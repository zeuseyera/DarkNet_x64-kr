...
void train_compare( char *cfgfile, char *weightfile )
{
	...
	while ( 1 )
	{
		...
		if ( i%100 == 0 )
		{
			...
			sprintf_s( buff, 256, "%s/%s_%d_minor_%d.weights", backup_directory, base, epoch, i );
...
{
	int k = 32;
	float EA = 1./(1+pow( 10, (b->elos[BunRyu] - a->elos[BunRyu])/400. ));
	float EB = 1./(1+pow( 10, (a->elos[BunRyu] - b->elos[BunRyu])/400. ));
	float SA = result ? 1 : 0;
	float SB = result ? 0 : 1;
	a->elos[BunRyu] += k*(SA - EA);
	b->elos[BunRyu] += k*(SB - EB);
}
...
void BattleRoyaleWithCheese( char *filename, char *weightfile )
{
	...
	for ( class = 0; class < classes; ++class )
	{
		...
		sprintf_s( buff, 256, "results/battle_%d.log", class );

		//FILE *outfp = fopen( buff, "w" );
		FILE *outfp; fopen_s( &outfp, buff, "w" );
