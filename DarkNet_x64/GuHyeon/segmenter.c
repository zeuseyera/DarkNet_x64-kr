...
/*
#ifdef WIN32			//  [6/27/2018 jobs]
#include <time.h>		// 윈도우 환경 [7/4/2018 jobs]
#else
#include <sys/time.h>	// 유닉스 환경 [7/4/2018 jobs]
#endif
*/
#include "gettimeofday.h"	// 윈도우 환경, timersub [7/3/2018 jobs]
					...
					, int display )
{
	...
	while ( get_current_batch( net ) < net->max_batches || net->max_batches == 0 )
	{
		...
		if ( display )
		{
			image tr = float_to_image( net->w/div
									, net->h/div
									, 80
									, train.y.vals[net->batch*(net->subdivisions-1)] );
			image im = float_to_image( net->w
									, net->h
									, net->c
...
void demo_segmenter( char *datacfg, char *cfg, char *weights, int cam_index, const char *filename )
{
	...
	while ( 1 )
	{
		...
		timersub( &tval_after, &tval_before, &tval_result );	// gettimeofday [7/3/2018 jobs]
