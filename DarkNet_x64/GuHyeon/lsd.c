...
*/
		...
		if ( i%10000==0 )
		{
			...
			save_weights( gnet, buff );
			//sprintf( buff, "%s/%s_%d.weights", backup_directory, abase, i );
			sprintf_s( buff, 256, "%s/%s_%d.weights", backup_directory, abase, i );
			save_weights( anet, buff );
		}
		...
		if ( i%1000==0 )
		{
			...
			save_weights( gnet, buff );
			//sprintf( buff, "%s/%s.backup", backup_directory, abase );
			sprintf_s( buff, 256, "%s/%s.backup", backup_directory, abase );
			save_weights( anet, buff );
		}
	}
...
/*			for ( z = 0; z < 100; ++z )
			...
			printf( "\n" );
			printf( "input: %f %f\n"
				, mean_array( gnet->input, x_size )
				, variance_array( gnet->input, x_size ) );
*/
...
/*			for ( k = 0; k < gnet->n; ++k )
			{
				...
				printf( "%d: %f %f\n"
					, k
					, mean_array( l.output, l.outputs*l.batch )
					, variance_array( l.output, l.outputs*l.batch ) );
			}
*/
